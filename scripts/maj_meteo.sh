...
docker compose run --rm --no-deps app MAJ_METEO >> "$LOG_FILE" 2>&1
...
if [ $EXIT_CODE -ne 0 ]; then
	#on log l'erreur
	echo "Erreur MAJ_METEO le $(date)" >> "$LOG_FILE"
	#envoie de l'erreur
	echo "Erreur MAJ_METEO le $(date)" | mail -s "ERREUR PREDICTELEC MAJ_METEO" famille.nony@sfr.fr
else
	echo "Fin MAJ_METEO : $(date)" >> "$LOG_FILE"
fi
